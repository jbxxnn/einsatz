...
// Configure route for larger body sizes (images)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file || !bookingRequestId) {
      return NextResponse.json({ error: 'Missing file or bookingRequestId' }, { status: 400 })
    }
    ...
    const filePath = `booking-requests/${bookingRequestId}/${fileName}`
